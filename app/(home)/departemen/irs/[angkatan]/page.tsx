...
// import { fetchMahasiswaByNim } from "@/data/departemen";
import { fetchAllMahasiswa } from "@/data/departemen";
import { groupByAngkatanBySemester } from "@/utils/functions";
...
export default async function Page() {
    ...
    // const MhsData = await fetchMahasiswaByNim(session!.user.user_metadata.no_induk,session!.user.user_metadata.angkatan);
    const MhsData = await fetchAllMahasiswa();

    const irsData = await fetchIrsByNim(params.nim);

    const groupAngkatanSem = groupByAngkatanBySemester(MhsData, irsData);
    return (
        <div>
        <br />
        <br />
            ...
            <div className="flex-col flex w-11/12">
                ...
                return (
                    ...
                    <div className="flex items-center bg-gray-200 w-full h-20 p-4">
                        <div>
                        <Link href={`/departemen/irs/${angkatan}/${semester}`}>
