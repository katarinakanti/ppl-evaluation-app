...
export default function UpdatePkl({ data }: { data: Pkl }) {
    ...
    return (
        ...
        <form action={dispatch} className="mx-auto bg-gray-100 w-10/12 h-fit">
            ...
            <div className="flex-col ml-8"> 
                ...
                {state.errors?.dosen_pembimbing && (
                    <div
                    id="dosen_pembimbing-error"
                    aria-live="polite"
                    className="mt-2 text-sm text-red-500"
                    >
                    {state.errors.dosen_pembimbing.map((error: string) => (
                        <p key={error}>{error}</p>
                    ))}
                    </div>
                )}
                ...
                <div className="flex items-center ml-28">
                    ...
                    <div>
                    <a
                        href={getFileUrl(data.upload_file)}
                        target="_blank"
                        className="bg-blue-100 hover:bg-blue-200 p-2"
                    >
                        Lihat file lama: {data.upload_file}
                    </a>
                    </div>
                </div>
                {state.errors?.file && (
